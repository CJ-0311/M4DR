...
class Config:
    # --- 文件路徑 ---
    BULK_EXPR_PATH = '~/jupyterlab/GSE140440_pc3/gdsc_t.csv'
    DRUG_SMILES_PATH = '~/jupyterlab/GSE247689/unseendrug/alldrugname_with_smiles_noolaparib.csv'
    DRUG_RESPONSE_PATH = '~/jupyterlab/GSE247689/unseendrug/gdsc100_noolaparib.csv'
    SINGLE_CELL_EXPR_PATH = '~/jupyterlab/GSE247689/GSE247689cov362_c_t5tpm.csv'
    BULK_PATHWAY_PATH = '~/jupyterlab/bulk_pathwayscoreMatrix.csv'
    SC_PATHWAY_PATH = '~/jupyterlab/GSE247689/GSE247689cov362_c_t5_pathwayscoreMatrix.csv'
    ...
    TEST_SIZE = 0.2
    RANDOM_STATE = 42
    OUTPUT_DIR = '/home/bio_wangxf/jupyterlab/GSE247689/outputs/'

    PRETRAIN_CKPT = os.path.join(OUTPUT_DIR, "pretrain_model.pth")
    FINETUNE_CKPT = os.path.join(OUTPUT_DIR, "finetune_model.pth")
    # --- 特徵維度 ---
    FP_SIZE = 2048  # 摩根指紋維度
...
# 新增: 加载训练好的模型
# =============================================================================
def load_trained_model(config, gene_dim, pathway_dim, fp_dim=2048, graph_node_dim=33):
    """加载训练好的模型"""
    model = FourTowerModel(gene_dim, pathway_dim, fp_dim, graph_node_dim).to(config.DEVICE)

    # 尝试加载微调后的模型，如果不存在则加载预训练模型
    if not load_checkpoint(model, config.FINETUNE_CKPT, config.DEVICE):
        print(f"Finetune checkpoint not found, trying pretrained model...")
        if not load_checkpoint(model, config.PRETRAIN_CKPT, config.DEVICE):
            print(f"No trained model found at {config.FINETUNE_CKPT} or {config.PRETRAIN_CKPT}")
            return None
...
# =============================================================================
@torch.no_grad()
def predict_unseen_drug(smiles, df_sc_expr, df_sc_pathway, model, config):
    """
    对未见药物进行单细胞水平预测

    Args:
        smiles: 新药SMILES字符串
        df_sc_expr: 单细胞基因表达DataFrame (索引为细胞名)
        df_sc_pathway: 单细胞通路得分DataFrame (索引为细胞名)
        model: 训练好的模型
        config: 配置对象

    Returns:
        DataFrame: 包含每个细胞的预测结果
    """
    print(f"Predicting for new drug with SMILES: {smiles}")
    ...
    drug_fp = smiles_to_fingerprint(smiles, n_bits=config.FP_SIZE).unsqueeze(0).to(config.DEVICE)

    # 将图转为batch输入
    drug_graph = drug_graph.to(config.DEVICE)
    batch_graph = Data(
        x=drug_graph.x,
        edge_index=drug_graph.edge_index,
        batch=torch.zeros(drug_graph.x.size(0), dtype=torch.long).to(config.DEVICE)
    )
    ...
    print(f"Processing {len(df_sc_expr)} single cells...")

    for i in range(len(df_sc_expr)):
        if i % 100 == 0 and i > 0:
            print(f"  Processed {i} cells...")

        gene = torch.FloatTensor(df_sc_expr.iloc[i].values).unsqueeze(0).to(config.DEVICE)
        path = torch.FloatTensor(df_sc_pathway.iloc[i].values).unsqueeze(0).to(config.DEVICE)

        # 使用目标域编码器（单细胞编码器）进行预测
        preds, _, _ = model(gene, path, batch_graph, drug_fp, domain='target')
        prob = torch.softmax(preds, dim=1)[:, 1].item()  # 输出敏感概率

        predictions.append(prob)
        cell_names.append(df_sc_expr.index[i])

    # 3. 创建结果DataFrame
    result = pd.DataFrame({
        "Cell": cell_names,
        "Sensitive_probability": predictions
    })

    # 添加分类标签
    result["Prediction"] = result["Sensitive_probability"].apply(
        lambda x: "Sensitive" if x > 0.5 else "Resistant"
    )

    # 按敏感概率降序排序
    result = result.sort_values("Sensitive_probability", ascending=False)

    print(f"Prediction complete. Top 5 predictions:")
    print(result.head())
...
class GraphTransformer(nn.Module):
    ...
    def __init__(self, input_dim, embed_dim=64, num_heads=4, num_layers=2, dropout=0.2):
        ...
        self.layers = nn.ModuleList(
            [GraphTransformerLayer(embed_dim, num_heads, dropout=dropout) for _ in range(num_layers)])
...
def evaluate(model, dataloader, device, domain='source'):
    ...
    with torch.no_grad():
        for i, (gene_expr, pathway_score, drug_graph_loader, drug_fp, labels) in enumerate(dataloader):
            ...
            gene_expr, pathway_score, drug_fp, labels = gene_expr.to(device), pathway_score.to(device), drug_fp.to(
...
def train_source_pretraining(model, train_loader, val_loader, optimizer, scheduler, config):
    checkpoint_dir = "/home/bio_wangxf/jupyterlab/GSE247689/checkpoints_test/pretrain_advanced_gdsc1.3"
...
# 修改：第二階段 - 領域自適應微調函數
# 修改：第二階段 - 領域自適應微調函數
def train_domain_adaptation(model, source_loader, target_loader, val_loader, optimizer, scheduler, adaptation_method,
                            config):
    ...
    for epoch in range(start_epoch, config.FINETUNE_EPOCHS):
        ...
        for i in range(len_dataloader):
            ...
            with torch.no_grad():
                source_gene_feat, source_pathway_feat = model(s_gene, s_path, domain='source', drug_graph=None,
                                                              drug_fp=None)
                target_gene_feat, target_pathway_feat = model(t_gene, t_path, domain='target', drug_graph=None,
                                                              drug_fp=None)
        ...
        if avg_T_loss < best_T_loss:
            ...
            print(f"  * New best Target Encector Loss: {best_T_loss:.4f}. Checkpoint saved to best_T_loss_model.")
        ...
        if avg_D_loss > best_D_loss:
            ...
            print(
                f"  * New best (highest) Discriminator Loss: {best_D_loss:.4f}. Checkpoint saved to best_D_loss_model.")
...
if __name__ == '__main__':
    ...
    def run_new_drug_inference():
        """单独运行新药推理"""
        print("\n" + "=" * 60)
        print("NEW DRUG INFERENCE MODULE")
        print("=" * 60)

        # 动态获取维度
        gene_dim = df_bulk_expr.shape[1]
        pathway_dim = df_bulk_pathway.shape[1]

        # 加载训练好的模型
        print("Loading trained model...")
        model = load_trained_model(cfg, gene_dim=gene_dim, pathway_dim=pathway_dim)

        if model is None:
            print("Error: No trained model found. Please train first or check checkpoint paths.")
            return

        # 新药SMILES示例
        # 这里可以替换成您想要测试的任何新药SMILES
        new_drug_smiles_examples = [
            
            "C1CC1C(=O)N2CCN(CC2)C(=O)C3=C(C=CC(=C3)CC4=NNC(=O)C5=CC=CC=C54)F" 
        ]

        for i, smiles in enumerate(new_drug_smiles_examples, 1):
            print(f"\n--- Predicting for Drug {i}: {smiles[:30]}... ---")

            # 执行预测
            result = predict_unseen_drug(
                smiles=smiles,
                df_sc_expr=df_sc_expr,
                df_sc_pathway=df_sc_pathway,
                model=model,
                config=cfg
            )
            ...
            if result is not None:
                # 保存结果
                output_path = os.path.join(cfg.OUTPUT_DIR, f"unseen_drug_{i}_predictions.csv")
                result.to_csv(output_path, index=False)
                print(f"Predictions saved to {output_path}")

                # 统计结果
                sensitive_count = (result["Prediction"] == "Sensitive").sum()
                resistant_count = (result["Prediction"] == "Resistant").sum()
                print(f"Sensitive cells: {sensitive_count} ({sensitive_count / len(result) * 100:.1f}%)")
                print(f"Resistant cells: {resistant_count} ({resistant_count / len(result) * 100:.1f}%)")
    ...
    # ======================================================================
    print("\nSelect mode:")
    print("1. Full training + new drug inference")
    print("2. Only new drug inference (skip training)")

    # 这里可以根据需要修改，比如从命令行参数读取
    mode = 2  # 默认只运行新药推理
    ...
    # 检查预训练检查点是否存在
    if not load_checkpoint(pretrain_model, cfg.PRETRAIN_CKPT, cfg.DEVICE):
        print("No pretrain checkpoint found, start training...")
        ...
        save_checkpoint(pretrain_model, cfg.PRETRAIN_CKPT)
    else:
        print("Pretrain checkpoint exists, skip pretraining.")
    ...
    for method in ["ADDA"]:
        ...
        # 加載預訓練權重
        checkpoint = torch.load(cfg.PRETRAIN_CKPT, map_location=cfg.DEVICE)
        finetune_model.load_state_dict(checkpoint)

        # 检查微调检查点是否存在
        if not load_checkpoint(finetune_model, cfg.FINETUNE_CKPT, cfg.DEVICE):
            print("No finetune checkpoint found, start finetuning...")
            ...
            save_checkpoint(finetune_model, cfg.FINETUNE_CKPT)
        else:
            print("Finetune checkpoint exists, skip finetuning.")
        ...
        # 检查使用哪个模型进行预测
        if load_checkpoint(finetune_model, cfg.FINETUNE_CKPT, cfg.DEVICE):
            print("Using finetuned model for prediction...")
            output_filename = os.path.join(cfg.OUTPUT_DIR, f"sc_drug_predictions_{method}_finetuned.csv")
            perform_inference_and_save(finetune_model, pred_loader, cfg.DEVICE, output_path=output_filename)
        elif load_checkpoint(finetune_model, cfg.PRETRAIN_CKPT, cfg.DEVICE):
            print("Using pretrained model (no finetune) for prediction...")
            output_filename = os.path.join(cfg.OUTPUT_DIR, f"sc_drug_predictions_{method}_pretrained.csv")
            perform_inference_and_save(finetune_model, pred_loader, cfg.DEVICE, output_path=output_filename)
        else:
            print("No checkpoint found, need to train first.")
