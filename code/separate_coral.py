...
def train_domain_adaptation(model, source_loader, target_loader, val_loader, optimizer, scheduler, adaptation_method, config):
    checkpoint_dir = os.path.join("/home/bio_wangxf/jupyterlab/checkpoints_test_advanced_gdsc1.3", adaptation_method)
    os.makedirs(checkpoint_dir, exist_ok=True)
    
    last_ckpt_path = os.path.join(checkpoint_dir, "last_checkpoint.pth")
    best_loss_ckpt_path = os.path.join(checkpoint_dir, "best_loss_checkpoint.pth")
    # For consistency, we keep this variable, though it won't be used
    best_auc_ckpt_path = os.path.join(checkpoint_dir, "best_auc_checkpoint.pth")

    adaptation_fn = loss_dict[adaptation_method]
    start_epoch, best_adapt_loss = 0, float('inf')

    # 1. 複製預訓練權重到 SC 編碼器
    model.copy_bulk_to_sc_encoders()

    # 2. 凍結除了 SC 編碼器以外的所有部分
    print("--- Freezing ALL modules except SC-Encoders for fine-tuning ---")
    ...
    # 3. 建立只包含 SC 編碼器參數的優化器
    trainable_params = list(model.sc_gene_tower.parameters()) + \
                       list(model.sc_pathway_tower.parameters())
    optimizer = optim.AdamW(trainable_params, lr=config.LR_FINETUNE, weight_decay=config.WEIGHT_DECAY)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.5)

    # --- 模型恢復邏輯 ---
    if os.path.exists(last_ckpt_path):
        print(f"--- Resuming fine-tuning for {adaptation_method} from checkpoint ---")
        checkpoint = torch.load(last_ckpt_path, map_location=config.DEVICE)
        # Note: We load the whole model state, but only SC encoders will be updated
        model.load_state_dict(checkpoint['model_state_dict'])
        start_epoch = checkpoint['epoch'] + 1
        best_adapt_loss = checkpoint.get('best_adapt_loss', float('inf'))

    print(f"\n--- Starting Phase 2: Fine-tuning with {adaptation_method} ---")
    print("--- Training SC-ENCODERS ONLY. All other modules are FROZEN. ---")
    
    for epoch in range(start_epoch, config.FINETUNE_EPOCHS):
        model.sc_gene_tower.train()
        model.sc_pathway_tower.train()
        
        total_adapt_loss = 0
        target_iter = iter(target_loader)

        for i, (s_gene, s_path, _, _, _) in enumerate(source_loader):
            try:
            ...
            if s_gene.size(0) != t_gene.size(0):
                continue
            ...
            # CORAL 不像 JAN 需要特殊處理，所以可以直接計算
            loss_adapt_gene = adaptation_fn(source_gene_feat, target_gene_feat)
            loss_adapt_pathway = adaptation_fn(source_pathway_feat, target_pathway_feat)
            
            total_loss = loss_adapt_gene + loss_adapt_pathway
            ...
            total_loss.backward()
            optimizer.step()
            total_adapt_loss += total_loss.item()

        avg_adapt_loss = total_adapt_loss / len(source_loader)
        
        print(f"Fine-tune Epoch {epoch + 1}/{config.FINETUNE_EPOCHS} | "
              f"Adapt Loss: {avg_adapt_loss:.4f} | "
              f"LR: {optimizer.param_groups[0]['lr']:.2e}")
        
        scheduler.step()

        # 只根據 best_adapt_loss 儲存模型
        if avg_adapt_loss < best_adapt_loss:
            best_adapt_loss = avg_adapt_loss
            # When saving, we save the state of the entire model,
            # which includes the newly trained sc_encoders and the frozen parts.
            torch.save({'model_state_dict': model.state_dict()}, best_loss_ckpt_path)
            print(f"  * New best Adapt Loss: {best_adapt_loss:.4f}. Checkpoint saved.")

        torch.save({
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'best_adapt_loss': best_adapt_loss
        }, last_ckpt_path)
    ...
    print(f"--- Phase 2: Fine-tuning Finished for {adaptation_method} ---")
