...
class Config:
    # --- 文件路徑 ---
    BULK_EXPR_PATH = '~/jupyterlab/hvg4000_gdsc.csv'
    ...
    TEST_SIZE = 0.2
    RANDOM_STATE = 42
    OUTPUT_DIR = '/home/bio_wangxf/jupyterlab/outputs/'
    # --- 特徵維度 ---
    FP_SIZE = 2048  # 摩根指紋維度
...
class DomainClassifier(nn.Module):
    ...
    def forward(self, features, alpha=1.0):
        # Apply gradient reversal layer before passing to the classifier
...
class FourTowerModel(nn.Module):
    def __init__(self, gene_dim, pathway_dim, fp_dim, graph_node_dim):
        ...
        # ==========================================================
        # 為 DANN 新增域分類器
        # 我們需要兩個分類器，分別用於基因特徵和通路特徵
        feature_dim = 256 # 您的編碼器的輸出維度
        self.gene_domain_classifier = DomainClassifier(input_dim=feature_dim)
        self.pathway_domain_classifier = DomainClassifier(input_dim=feature_dim)
        # ==========================================================
        # --- END OF MODIFICATION ---
        # ==========================================================
...
def train_source_pretraining(model, train_loader, val_loader, optimizer, scheduler, config):
    checkpoint_dir = "/home/bio_wangxf/jupyterlab/checkpoints_test/pretrain_advanced_gdsc1.3"
...
# 修改：第二階段 - 領域自適應微調函數
# 修改：第二階段 - 領域自適應微調函數
# --- MODIFICATION: Replace fine-tuning logic with DANN ---
def train_domain_adaptation(model, source_loader, target_loader, val_loader, optimizer, scheduler, adaptation_method, config):
    checkpoint_dir = os.path.join("/home/bio_wangxf/jupyterlab/checkpoints_test_advanced_gdsc1.3", "DANN")
    os.makedirs(checkpoint_dir, exist_ok=True)
    
    last_ckpt_path = os.path.join(checkpoint_dir, "last_checkpoint.pth")
    best_loss_ckpt_path = os.path.join(checkpoint_dir, "best_loss_checkpoint.pth")
    best_auc_ckpt_path = os.path.join(checkpoint_dir, "best_auc_checkpoint.pth")  # not used, kept for API consistency

    start_epoch = 0
    # 目标值：二分类随机猜测时的 CrossEntropy ≈ 0.693
    target_D_loss = 0.693
    best_diff = float("inf")
    ...
    # 2. 凍結所有非 SC-Encoder 的部分
    print("--- Freezing ALL modules except SC-Encoders for DANN fine-tuning ---")
    ...
    optimizer_D = optim.AdamW(
        list(model.gene_domain_classifier.parameters()) + list(model.pathway_domain_classifier.parameters()),
        lr=config.LR_FINETUNE, weight_decay=config.WEIGHT_DECAY
    )

    criterion_D = nn.BCEWithLogitsLoss()

    print(f"\n--- Starting Phase 2: Fine-tuning with DANN ---")
    print("--- Training SC-ENCODERS to fool Domain Classifiers. All other modules are FROZEN. ---")
    ...
    for epoch in range(start_epoch, config.FINETUNE_EPOCHS):
        ...
        for i in range(len_dataloader):
            ...
            # === Step 1: Train Domain Classifiers ===
            ...
            d_source_gene = model.gene_domain_classifier(source_gene_feat.detach(), -1)
            d_target_gene = model.gene_domain_classifier(target_gene_feat.detach(), -1)
            d_source_path = model.pathway_domain_classifier(source_pathway_feat.detach(), -1)
            d_target_path = model.pathway_domain_classifier(target_pathway_feat.detach(), -1)

            domain_source = torch.zeros(s_gene.size(0), 1).to(config.DEVICE)
            domain_target = torch.ones(t_gene.size(0), 1).to(config.DEVICE)

            loss_D_gene = criterion_D(d_source_gene, domain_source) + criterion_D(d_target_gene, domain_target)
            loss_D_path = criterion_D(d_source_path, domain_source) + criterion_D(d_target_path, domain_target)
            loss_D = loss_D_gene + loss_D_path
            loss_D.backward()
            optimizer_D.step()
            total_D_loss += loss_D.item()

            # === Step 2: Train SC-Encoders ===
            optimizer_G.zero_grad()
            target_gene_feat, target_pathway_feat = model(t_gene, t_path, domain='target', drug_graph=None, drug_fp=None)
            d_target_gene_adv = model.gene_domain_classifier(target_gene_feat) 
            d_target_path_adv = model.pathway_domain_classifier(target_pathway_feat) 
            loss_G_gene = criterion_D(d_target_gene_adv, domain_source)
            loss_G_path = criterion_D(d_target_path_adv, domain_source)
            loss_G = loss_G_gene + loss_G_path
            loss_G.backward()
            optimizer_G.step()
            total_G_loss += loss_G.item()

        avg_D_loss = total_D_loss / len_dataloader
        avg_G_loss = total_G_loss / len_dataloader

        print(f"Fine-tune Epoch {epoch + 1}/{config.FINETUNE_EPOCHS} | "
              f"DomainClassifier Loss (D_loss): {avg_D_loss:.4f} | "
              f"FeatureExtractor Loss (G_loss): {avg_G_loss:.4f} | "
              f"LR: {optimizer_G.param_groups[0]['lr']:.2e}")
        
        # ✅ 保存最接近 0.69 的 checkpoint
        diff = abs(avg_D_loss - target_D_loss)
        if diff < best_diff:
            best_diff = diff
            torch.save({'model_state_dict': model.state_dict()}, best_loss_ckpt_path)
            print(f"  * New best Domain Loss (closest to 0.693): {avg_D_loss:.4f}. Checkpoint saved.")

        torch.save({'epoch': epoch, 'model_state_dict': model.state_dict()}, last_ckpt_path)

    for p in model.parameters(): 
        p.requires_grad = True
    print(f"--- Phase 2: Fine-tuning Finished for DANN ---")
    return best_auc_ckpt_path, best_loss_ckpt_path
...
if __name__ == '__main__':
    ...
    # --- Phase 1: 監督式預訓練 ---
    pretrain_checkpoint_path = "/home/bio_wangxf/jupyterlab/checkpoints_test/pretrain_advanced_gdsc1.3/best_auc_checkpoint.pth"
    ...
    for method in ["DANN"]:
        ...
        best_auc_path, best_loss_path = train_domain_adaptation(
